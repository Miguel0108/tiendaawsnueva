...
public class ReporteDAO{
	...
	public ArrayList<ReporteVentasClienteDTO> listarVentasPorCliente(){
		...
		try {
			String sql = "SELECT c.cedula_cliente, c.nombre_cliente, SUM(v.valor_venta) FROM cliente AS c LEFT JOIN venta AS v ON v.cedula_cliente = c.cedula_cliente GROUP BY c.cedula_cliente";
			ps = connect.prepareStatement(sql);
			rs = ps.executeQuery();
